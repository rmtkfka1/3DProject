...
void Core::RenderEnd()
{
	D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
		_rtvBuffer[_backBufferIndex].Get(),
		D3D12_RESOURCE_STATE_RENDER_TARGET, // ø‹¡÷ ∞·∞˙π∞
		D3D12_RESOURCE_STATE_PRESENT); // »≠∏È √‚∑¬
...
void Core::CreateSwapChain()
{
	...
	sd.BufferDesc.Width = static_cast<uint32>(_info.width); // πˆ∆€¿« «ÿªÛµµ ≥ ∫Ò
	sd.BufferDesc.Height = static_cast<uint32>(_info.height); // πˆ∆€¿« «ÿªÛµµ ≥Ù¿Ã
	sd.BufferDesc.RefreshRate.Numerator = 60; // »≠∏È ∞ªΩ≈ ∫Ò¿≤
	sd.BufferDesc.RefreshRate.Denominator = 1; // »≠∏È ∞ªΩ≈ ∫Ò¿≤
	sd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // πˆ∆€¿« µΩ∫«√∑π¿Ã «¸Ωƒ
	sd.BufferDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	sd.BufferDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	sd.SampleDesc.Count = 1; // ∏÷∆º ª˘«√∏µ OFF
	sd.SampleDesc.Quality = 0;
	sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT; // »ƒ∏È πˆ∆€ø° ∑ª¥ı∏µ«“ ∞Õ 
	sd.BufferCount = SWAP_CHAIN_BUFFER_COUNT; // ¿¸∏È+»ƒ∏È πˆ∆€
	sd.OutputWindow = _info.hwnd;
	sd.Windowed = true;
	sd.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD; // ¿¸∏È »ƒ∏È πˆ∆€ ±≥√º Ω√ ¿Ã¿¸ «¡∑π¿” ¡§∫∏ πˆ∏≤
